...
async def test_verify_totp_success(mock_settings, mock_firestore_client):
  ...
  mock_snapshot.to_dict.return_value = {"totp_enabled": True, "totp_secret_encrypted": encrypted_secret, "last_used_otp": "000000"}
...
async def test_verify_totp_failure_invalid_token(mock_settings, mock_firestore_client):
  ...
  mock_snapshot.to_dict.return_value = {"totp_enabled": True, "totp_secret_encrypted": encrypted_secret, "last_used_otp": "000000", "totp_failed_attempts": 0}
  mock_doc_ref.get.return_value = mock_snapshot
...
async def test_verify_totp_replay_attack(mock_settings, mock_firestore_client):
  ...
  mock_snapshot.to_dict.return_value = {"totp_enabled": True, "totp_secret_encrypted": encrypted_secret, "last_used_otp": token}
  mock_doc_ref.get.return_value = mock_snapshot
...
async def test_verify_totp_not_enabled(mock_settings, mock_firestore_client):
  ...
  mock_snapshot = MagicMock()
  mock_snapshot.exists = True
  mock_snapshot.to_dict.return_value = {"totp_enabled": False}
  mock_doc_ref.get.return_value = mock_snapshot
...
async def test_verify_totp_rate_limiting(mock_settings, mock_firestore_client):
  ...
  mock_snapshot.to_dict.return_value = {"totp_enabled": True, "totp_secret_encrypted": encrypted_secret, "totp_failed_attempts": RATE_LIMIT_attempts, "totp_last_failed_at": last_failed.isoformat(), "last_used_otp": "000000"}
  mock_doc_ref.get.return_value = mock_snapshot
