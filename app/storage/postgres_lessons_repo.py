  ...
  async def create_subjective_input_widgets(self, records: list[SubjectiveInputWidgetRecord]) -> list[SubjectiveInputWidgetRecord]:
    ...
    async with self._session_factory() as session:
      if not records:
        ...
        db_widgets = [SubjectiveInputWidget(section_id=r.section_id, widget_type=r.widget_type, ai_prompt=r.ai_prompt, wordlist=r.wordlist) for r in records]
        session.add_all(db_widgets)
        await session.flush()
        created_records = [SubjectiveInputWidgetRecord(id=widget.id, section_id=widget.section_id, widget_type=widget.widget_type, ai_prompt=widget.ai_prompt, wordlist=widget.wordlist, created_at=str(widget.created_at)) for widget in db_widgets]
      await session.commit()
      return created_records
