...
def _error_payload(detail: str, settings: Settings, *, error: str | None = None, logs: list[str] | None = None) -> dict[str, Any]:
...
async def global_exception_handler(request: Request, exc: Exception) -> DecimalJSONResponse:
  ...
  return DecimalJSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content=_error_payload("Internal Server Error", settings, error=str(exc)))
...
async def orchestration_exception_handler(request: Request, exc: OrchestrationError) -> DecimalJSONResponse:
  ...
  return DecimalJSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content=_error_payload("Orchestration failed", settings, error=str(exc), logs=exc.logs))
