...
class CoachAgent(BaseAgent[dict[str, Any], list[int]]):
  ...
  async def run(self, input_data: dict[str, Any], ctx: JobContext) -> list[int]:
    ...
    if not raw_user_id:
      raise RuntimeError("Coach missing user_id metadata for quota reservation.")
    try:
      reservation_user_id = uuid.UUID(str(raw_user_id))
    except ValueError as exc:
      raise RuntimeError("Coach received invalid user_id metadata.") from exc

    # Resolve runtime configuration for the coach quota limit.
    async with session_factory() as session:
      user = await get_user_by_id(session, reservation_user_id)
      if user is None:
        raise RuntimeError("Coach quota reservation failed: user not found.")
      tier_id, _tier_name = await get_user_subscription_tier(session, user.id)
      settings = (ctx.metadata or {}).get("settings")
      if settings is None:
        raise RuntimeError("Coach missing settings metadata for quota resolution.")
      ...
      reservation_limit = int(runtime_config.get("limits.coach_sections_per_month") or 0)
      if reservation_limit <= 0:
        raise QuotaExceededError("coach.generate quota disabled")

    try:
      # Reserve monthly coach quota before generating audio.
      async with session_factory() as session:
        # Build quota metadata for audit logging.
        reserve_metadata = {"job_id": str(ctx.job_id), "section_index": int(section_index)}
        await reserve_quota(session, user_id=reservation_user_id, metric_key="coach.generate", period=QuotaPeriod.MONTH, quantity=1, limit=reservation_limit, job_id=str(ctx.job_id), section_index=int(section_index), metadata=reserve_metadata)
      ...
      async with session_factory() as session:
        ...
        await commit_quota_reservation(session, user_id=reservation_user_id, metric_key="coach.generate", period=QuotaPeriod.MONTH, quantity=1, limit=reservation_limit, job_id=str(ctx.job_id), section_index=int(section_index), metadata=commit_metadata)
      return audio_ids
    except Exception:  # noqa: BLE001
      # Release quota reservation when coach generation fails.
      logger.error("Coach failed during execution.", exc_info=True)
      if reservation_active and reservation_user_id is not None:
        try:
          async with session_factory() as session:
            # Build quota metadata for audit logging.
            release_metadata = {"job_id": str(ctx.job_id), "section_index": int(section_index), "reason": "coach_failed"}
            await release_quota_reservation(
              session, user_id=reservation_user_id, metric_key="coach.generate", period=QuotaPeriod.MONTH, quantity=1, limit=reservation_limit, job_id=str(ctx.job_id), section_index=int(section_index), metadata=release_metadata
            )
        except Exception:  # noqa: BLE001
          logger.error("Coach failed to release coach quota reservation.", exc_info=True)
      raise
