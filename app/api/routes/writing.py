...
async def create_writing_check(  # noqa: B008
  ...
  """Synchronously check a writing task response."""
  ...
  try:
    ...
    # Release quota on system failure
    await release_quota_reservation(db_session, user_id=current_user.id, metric_key="writing.check", period=QuotaPeriod.MONTH, quantity=1, limit=writing_checks_per_month, job_id=job_id, metadata={"job_id": job_id, "reason": "writing_failed"})
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Writing check failed.") from exc
