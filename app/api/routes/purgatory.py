...
@router.get("/purgatory")
async def get_purgatory_status(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:  # noqa: B008
  """
  Get the current user's approval status while access is pending.
  """
  # Provide status without requiring an approved account.
