...
async def get_lesson_media(lesson_id: str, image_name: str, db_session: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_active_user), settings: Settings = Depends(get_settings)) -> Response:  # noqa: B008
  ...
  if _IMAGE_NAME_RE.match(image_name):
    stmt = (
      select(Illustration)
    ...
    direct_resource_stmt = (
      ...
      .join(Section, Section.illustration_id == Illustration.id)
      .join(Lesson, Lesson.lesson_id == Section.lesson_id)
      .where(Lesson.lesson_id == lesson_id, Lesson.user_id == user_id, Lesson.is_archived.is_(False), Illustration.public_id == image_name, Illustration.status == "completed", Illustration.is_archived.is_(False))
      .limit(1)
    )
  ...
  if illustration is None and not _IMAGE_NAME_RE.match(image_name):
    mapping_stmt = (
      ...
      .where(
        ...
        SubsectionWidget.public_id == image_name,
    ...
    if mapping is not None and mapping.widget_id is not None:
      ...
      if mapped_resource_id:
        illustration_stmt = select(Illustration).where(Illustration.public_id == mapped_resource_id, Illustration.creator_id == user_id, Illustration.status == "completed", Illustration.is_archived.is_(False)).limit(1)
        ...
        if illustration is None:
          ...
          if mapped_legacy_id is not None:
            legacy_stmt = select(Illustration).where(Illustration.id == mapped_legacy_id, Illustration.creator_id == user_id, Illustration.status == "completed", Illustration.is_archived.is_(False)).limit(1)
...
async def get_widget_illustration(
  ...
  illustration_stmt = select(Illustration).where(Illustration.public_id == mapped_widget_id, Illustration.status == "completed", Illustration.is_archived.is_(False), Illustration.creator_id == str(current_user.id)).limit(1)
  illustration_result = await db_session.execute(illustration_stmt)
  illustration = illustration_result.scalar_one_or_none()
  if illustration is None:
    try:
      ...
      mapped_legacy_id = None
    if mapped_legacy_id is not None:
      legacy_stmt = select(Illustration).where(Illustration.id == mapped_legacy_id, Illustration.status == "completed", Illustration.is_archived.is_(False), Illustration.creator_id == str(current_user.id)).limit(1)
      legacy_result = await db_session.execute(legacy_stmt)
      illustration = legacy_result.scalar_one_or_none()
  if illustration is None:
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Media not found")
