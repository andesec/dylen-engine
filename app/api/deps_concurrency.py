...
async def check_concurrency_limit(
    ...
    stmt = select(UserTierOverride).where(
        and_(
            UserTierOverride.user_id == user.id,
            UserTierOverride.expires_at > func.now()
        )
    ).order_by(UserTierOverride.id.desc()).limit(1)
    ...
    query = select(func.count(Job.job_id)).where(
        Job.user_id == str(user.id),
        Job.status.in_(active_statuses),
        or_(
            Job.ttl.is_(None),
            Job.ttl > current_time
        )
    )
    ...
    if feature == "lesson":
        ...
        query = query.where(
            or_(
                Job.target_agent.is_(None),
                Job.target_agent.in_(["lesson", "planner", "orchestrator", "lesson_orchestrator"])
            )
        )
    ...
    if active_count >= limit:
        raise HTTPException(
            ...
            detail=f"Concurrency limit reached for {feature}. Limit: {limit}, Active: {active_count}. Please wait for your current request to complete."
        )
