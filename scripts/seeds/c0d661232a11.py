"""Ensure all known permission slugs exist and are assigned to Super Admin."""
...
# Keep this list aligned with authorization checks in app/core/security.py and routes.
_KNOWN_PERMISSION_DEFINITIONS: tuple[tuple[str, str, str], ...] = (
  ("user:manage", "Manage Users", "List users and update roles/statuses."),
...
async def seed(connection: AsyncConnection) -> None:
  """Upsert known permissions and grant every permission to Super Admin."""
  # Verify required RBAC tables and columns exist before running upserts.
  ...
  # Resolve the Super Admin role id and exit safely if the role is absent.
  ...
  # Upsert all known permission slugs so authorization checks never reference missing rows.
  ...
  for slug, display_name, description in _KNOWN_PERMISSION_DEFINITIONS:
