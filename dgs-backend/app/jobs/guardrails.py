...
def maybe_truncate_result_json(result_json: dict[str, Any] | None) -> dict[str, Any] | None:
  ...
  return {"truncated": True, "preview": preview[:MAX_RESULT_BYTES], "message": "Result exceeded storage item size limit and was truncated."}
...
def maybe_truncate_artifacts(artifacts: dict[str, Any] | None) -> dict[str, Any] | None:
  ...
  return {"truncated": True, "preview": preview[:MAX_ARTIFACT_BYTES], "message": "Artifacts exceeded storage item size limit and were truncated."}


def enforce_item_size_guardrails(item: MutableMapping[str, Any], *, max_bytes: int = MAX_ITEM_BYTES, skip_size_check: bool = False) -> MutableMapping[str, Any]:
  """
