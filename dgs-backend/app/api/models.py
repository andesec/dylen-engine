...
class GenerateLessonRequest(BaseModel):
    ...
    teaching_style: (
        list[Literal["conceptual", "theoretical", "practical"]] | None
    ) = Field(
        default=None,
    ...
    def validate_depth_style_constraint(self) -> GenerateLessonRequest:
        if self.depth == "highlights" and self.teaching_style:
            if len(self.teaching_style) == 3:
                raise ValueError(
                    "Cannot select all teaching styles when depth is 'highlights'."
                )
        return self
...
class WritingCheckRequest(BaseModel):
    ...
    criteria: dict[str, Any] = Field(
        description="The evaluation criteria from the lesson."
    )
...
class JobStatusResponse(BaseModel):
    ...
    retry_count: StrictInt | None = Field(
        default=None, ge=0, description="Retry attempts already used for this job."
    )
    max_retries: StrictInt | None = Field(
        default=None, ge=0, description="Maximum retry attempts allowed for this job."
    )
    ...
    completed_steps: StrictInt | None = Field(
        default=None, ge=0, description="Completed progress steps when available."
    )
