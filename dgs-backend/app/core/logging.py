...
def _build_handlers(
    settings: Settings,
) -> tuple[logging.Handler, logging.Handler, Path]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to create log directory at {log_dir}: {exc}"
        ) from exc
...
def setup_logging(settings: Settings) -> Path:
    ...
    logging.basicConfig(
        level=logging.DEBUG, handlers=[stream_handler, file_handler], force=True
    )
    ...
    if not log_path.exists():
        raise RuntimeError(
            f"Logging initialization failed; log file missing at {log_path}"
        )
    return log_path
