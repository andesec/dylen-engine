        ...
        *,
        ...
        def _report_progress(
            ...
            if progress_callback:
                progress_callback(
                    phase_name, subphase, messages, advance, partial_json, section_progress
                )
        ...
        async def _process_structured_section(
            *,
            ...
            if structured.validation_errors and not enable_repair:
                ...
                error_message = (
                    f"Section {section_index} failed validation: {structured.validation_errors}"
                )
            ...
            if enable_repair:
                ...
                try:
                    ...
                    error_message = (
                        f"Repairer failed for section {section_index}/{section_count}: {exc}"
                    )
                    logs.append(error_message)
                    _report_progress(
                        "transform",
            ...
            _report_progress(
                "transform", validate_subphase, [f"Section {section_index} validated."]
            )
        ...
        target_section_count = (
            len(target_sections) if target_sections is not None else section_count
        )
        ...
        for plan_section in lesson_plan.sections:
            ...
            if merge_enabled:
                ...
                gather_msg = (
                    f"Gathering section {section_index}/{section_count}: {plan_section.title}"
                )
                _report_progress(
                    "collect",
                    gather_subphase,
                ...
                try:
                    ...
                    error_message = (
                        f"Gatherer failed section {section_index}/{section_count}: {exc}"
                    )
                    logs.append(error_message)
        ...
        if not merge_enabled:
            ...
            for section_index in section_indexes:
                ...
                try:
                    ...
                    error_message = (
                        f"Structurer failed for section {section_index}/{section_count}: {exc}"
                    )
                    logs.append(error_message)
