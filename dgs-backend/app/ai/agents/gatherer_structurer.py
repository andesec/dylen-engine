    ...
    async def run(self, input_data: PlanSection, ctx: JobContext) -> StructuredSection:
        ...
        with llm_call_context(
            ...
            if self._model.supports_structured_output and structured_output:
                schema = self._schema_service.sanitize_schema(
                    schema, provider_name=self._provider_name
                )
                try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            "Merged gatherer-structurer retry failed to parse JSON: %s", retry_exc
                        )
                        raise RuntimeError(
                            f"Failed to parse merged section JSON after retry: {retry_exc}"
                        ) from retry_exc
